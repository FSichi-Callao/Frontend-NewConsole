...
export const TextInput = ({ value, setValue, inputTitle, inputName, keyPressEvent, placeholder, col }) => {
    return (
        <div className={`col-md-6 col-sm-12 col-lg-${col ? col : '3'}`}>
            <label for="phone" className=" col-form-label">{inputTitle}</label>
            <input
                type="text"  data-testid="textbox" className="form-control required dni center-block" name={inputName} placeholder={placeholder}
                value={value} onChange={(e) => setValue(e.target.value)} onKeyPress={keyPressEvent}
            />
        </div>
    )
}
